...
$cliente = $clienteController->viewCliente($_SESSION['idCliente']);
$usuario = $usuarioController->viewUsuario($_SESSION['idUsuario']);

if (isset($_GET['action']) && $_GET['action'] == 'fetch_data') {
    // Combine os dados de cliente e usuario em um único array
    $response = [
        'cliente' => $cliente,
        'usuario' => $usuario
    ];
...
<body>
    ...
    <div class="form-container">
        ...
        <div class="form">
            ...
            <!-- Formulário para os dados pessoais -->
            <form id="form-dados" method="post" onsubmit="return validaData()">
                <div class="input-box">
            ...
            function verificarCodigo() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: 'editar-cliente.php?action=validar_codigo',  // Ação para validar o código
                            method: 'POST',
                            data: {
                            ...
                            success: function(response) {
                                ...
                                console.log('Resposta do servidor:', response); // Log da resposta para debug
                                    ...
                                    $.ajax({
                                        url: 'editar-cliente.php?action=update',
                                        method: 'GET',
                                        data: {
                                            updateUsuario: true,
        ...
        function verificarCodigoSenha() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // Faz a requisição AJAX para validar o código
                    $.ajax({
                        url: 'editar-cliente.php?action=validar_codigo',
                        method: 'POST',
                        data: {
                            codigoDigitado: codigoDigitado,
                            idUsuario: " . $_SESSION['idUsuario'] . "
                        ...
                        success: function(response) {
                            if (response === 'valido') {
                                ...
                                $.ajax({
                                    url: 'editar-cliente.php?action=update_senha',
                                    method: 'POST',
                                    data: {
    ...
    <script src="js/valida-edit-cliente.js"></script>
    <script src="js/valida-enviar.js"></script>

    <script>
        function loadData() {
            $.ajax({
                url: 'editar-cliente.php?action=fetch_data',
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    if (data.cliente && data.usuario) {
                        // Preenche os campos de dados do cliente
                        var cliente = data.cliente[0];
                        $('#nomeCliente').val(cliente.nome_cliente);
                        $('#nascCliente').val(cliente.data_nasc);
                        $('#cpfCliente').val(cliente.cpf_cliente);

                        // Preenche os campos de dados do usuário
                        var usuario = data.usuario[0];
                        $('#emailUsuario').val(usuario.email_usuario); // Campo de email
                    }
                },
