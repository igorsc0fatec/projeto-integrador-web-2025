...
if (isset($_GET['action']) && $_GET['action'] == 'fetch_data') {
    ...
    if (isset($_POST['pesq']) && !empty($_POST['pesq'])) {
        $massas = $massaController->pesquisaMassa();
    } else {
        $massas = $massaController->viewMassa($_SESSION['idConfeitaria']);
    }

    echo json_encode($massas);
    exit;
}

if (isset($_GET['id'])) {
    header('Content-Type: application/json');
    echo json_encode($massaController->deleteMassa($_GET['id']));
    exit;
}
?>
...
<body>
    ...
    if (isset($_POST['submit'])) {
        if ($massaController->verificaMassa()) {
            echo "
                    <script>
                        Swal.fire({
                        title: 'Erro ao cadastrar massa!',
            ...
            if ($massaController->addMassa()) {
                echo "
                            <script>
                                Swal.fire({
                                title: 'Massa cadastrada com sucesso!',
                                icon: 'success',
                                confirmButtonText: 'OK'
                                });
                            </script>";
    ...
    <div>
        <h2>Sua lista de Massas</h2>

        <form id="search-form" method="post">
            <div class="pesquisa">
                <label for="pesq">Buscar Massa</label>
                <div class="input-wrapper">
                    <input id="pesq" type="text" name="pesq" placeholder="Digite o nome da massa/ex: chocolate"
                        required>
    ...
    <script>
        $(document).ready(function () {
            function loadData(query = '') {
                $.ajax({
                    url: 'cadastrar-massa.php?action=fetch_data',
                    type: 'POST',
                    data: { pesq: query },
                    dataType: 'json',
                    success: function (data) {
                        var tableBody = $('#massas');
                        tableBody.empty();
                        if (data.length === 0) {
                            tableBody.append('<tr><td colspan="3">Você não tem nenhuma massa no momento</td></tr>');
                        } else {
                            data.forEach(function (massa) {
                                var row = $('<tr></tr>');
                                row.append('<td>' + massa.desc_massa + '</td>');
                                row.append('<td>' + massa.valor_por_peso + '</td>');
                                row.append('<td><button onclick="confirmarExclusao(' + massa.id_massa + ')">Excluir</button></td>');
                                row.append('<td><button onclick="confirmarEdicao(' + massa.id_massa + ',\'' + massa.desc_massa + '\',\'' + massa.valor_por_peso + '\')">Editar</button></td>');
        ...
        function confirmarExclusao(idMassa) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'cadastrar-massa.php?id=' + idMassa,
                        type: 'POST',
                        data: { idMassa: idMassa },
                        success: function (response) {
                            Swal.fire(
                                'Excluído!',
                                'A massa foi excluída.',
                                'success',
                            ).then(() => {
                        ...
                        error: function (jqXHR, textStatus, errorThrown) {
                            Swal.fire(
                                'Erro!',
                                'Ocorreu um erro ao excluir a massa.',
                                'error'
                            );
                        }
                    });
        ...
        function confirmarEdicao(idMassa, descMassa, valorPeso) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `editar-massa.php?id=${idMassa}&desc=${descMassa}&valor=${valorPeso}`;
