...
if (!isset($_SESSION['idConfeitaria'])) {
    ...
    exit();
} else if (isset($_SESSION['idTipoUsuario']) && $_SESSION['idTipoUsuario'] != 3) {
    echo "<script language='javascript' type='text/javascript'> window.location.href='login-confeitaria.php'</script>";
    exit();
}
...
$idUsuario = $_GET['u'];
$idCliente = $_GET['c'];
$dadosCliente = $controllerConfig->cliente->viewCliente($idCliente);
...
$statusConfeitaria = $controllerConfig->chat->online($idUsuario);
    ...
    <div class="chat-container">
        <div class="sidebar">
            <div class="profile-image">
                <img id="confeitaria-img" src="../assets/img-site/icon.png" alt="Foto de Perfil">
                alt="<?php echo $dadosCliente[0]['nome_cliente'] ?>">
            </div>
            <h2 id="confeitaria-nome"><?php echo htmlspecialchars($dadosCliente[0]['nome_cliente']); ?></h2>
            <p id="confeitaria-online-status">
                <?php
                if ($statusConfeitaria == "online") {
                    echo '<span style="color: green;">● Online</span>';
                } else {
                    echo '<span style="color: gray;">○ ' . htmlspecialchars($statusConfeitaria) . '</span>';
                }
                ?>
            </p>
        </div>
        ...
        <div class="chat-content">
            <div class="chat-header">
                Chat com <?php echo htmlspecialchars($dadosCliente[0]['nome_cliente']); ?>
            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            function loadMensagens() {
                $.ajax({
                    url: 'chat-confeitaria.php?action=fetch_mensagens&u=<?php echo $idUsuario; ?>&c=<?php echo $idCliente; ?>',
                    type: 'GET',
                    dataType: 'json',
