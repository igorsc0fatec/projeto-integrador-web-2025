...
if (isset($_GET['action']) && $_GET['action'] == 'fetch_data') {
    ...
    if (isset($_POST['pesq']) && !empty($_POST['pesq'])) {
        $decoracoes = $decoracaoController->pesquisaDecoracao();
    } else {
        $decoracoes = $decoracaoController->viewDecoracao($_SESSION['idConfeitaria']);
    }

    echo json_encode($decoracoes);
    exit;
}

if (isset($_GET['id'])) {
    header('Content-Type: application/json');
    echo json_encode($decoracaoController->deleteDecoracao($_GET['id']));
    exit;
}
?>
...
<body>
    ...
    if (isset($_POST['submit'])) {
        if ($decoracaoController->verificaDecoracao()) {
            echo "
                <script>
                    Swal.fire({
                    title: 'Erro ao cadastrar decoração!',
            ...
            if ($decoracaoController->addDecoracao()) {
                echo "
                    <script>
                        Swal.fire({
                        title: 'Decoração cadastrada com sucesso!',
                        icon: 'success',
                        confirmButtonText: 'OK'
                        });
                    </script>";
    ...
    <div>
        <h2>Sua lista de Decorações</h2>

        <form id="search-form" method="post">
            <div class="pesquisa">
                <label for="pesq">Buscar Decoração</label>
                <div class="input-wrapper">
                    <input id="pesq" type="text" name="pesq" placeholder="Digite o nome da decoração/ex: aniversário"
                        required>
    ...
    <script>
        $(document).ready(function () {
            function loadData(query = '') {
                $.ajax({
                    url: 'cadastrar-decoracao.php?action=fetch_data',
                    type: 'POST',
                    data: { pesq: query },
                    dataType: 'json',
                    success: function (data) {
                        var tableBody = $('#decoracoes');
                        tableBody.empty();
                        if (data.length === 0) {
                            tableBody.append('<tr><td colspan="3">Você não tem nenhuma decoração no momento</td></tr>');
                        } else {
                            data.forEach(function (decoracao) {
                                var row = $('<tr></tr>');
                                row.append('<td>' + decoracao.desc_decoracao + '</td>');
                                row.append('<td>' + decoracao.valor_por_peso + '</td>');
                                row.append('<td><button onclick="confirmarExclusao(' + decoracao.id_decoracao + ')">Excluir</button></td>');
                                row.append('<td><button onclick="confirmarEdicao(' + decoracao.id_decoracao + ',\'' + decoracao.desc_decoracao + '\',\'' + decoracao.valor_por_peso + '\')">Editar</button></td>');
        ...
        function confirmarExclusao(idDecoracao) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'cadastrar-decoracao.php?id=' + idDecoracao,
                        type: 'POST',
                        data: { idDecoracao: idDecoracao },
                        success: function (response) {
                            Swal.fire(
                                'Excluído!',
                                'A decoração foi excluída.',
                                'success',
                            ).then(() => {
                        ...
                        error: function (jqXHR, textStatus, errorThrown) {
                            Swal.fire(
                                'Erro!',
                                'Ocorreu um erro ao excluir a decoração.',
                                'error'
                            );
                        }
                    });
        ...
        function confirmarEdicao(idDecoracao, descDecoracao, valorPeso) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `editar-decoracao.php?id=${idDecoracao}&desc=${descDecoracao}&valor=${valorPeso}`;
